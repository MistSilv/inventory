            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                @foreach($stocktaking->regionStocktakings as $regionStock)
                    <a href="{{ route('region_stocktakings.censuses.index', $regionStock) }}">
                        <div class="bg-gray-900 border border-amber-700 rounded-lg p-4 shadow hover:shadow-lg transition">
                            <h3 class="text-lg font-bold text-amber-400">{{ $regionStock->region->name ?? '-' }}</h3>
                            <p class="text-sm text-gray-400">Code: {{ $regionStock->region->code ?? '-' }}</p>
                        </div>
                    </a>
                @endforeach
            </div>
